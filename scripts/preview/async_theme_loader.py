    ...
    def run(self):
        ...
        try:
            ...
            if cached_stylesheet:
                self.status_update.emit(f"Loaded theme from cache: {theme_name} ({variant})")
            ...
            if raw_stylesheet:
                self.status_update.emit(f"Processing cached stylesheet: {theme_name} ({variant})")
                ...
                self.status_update.emit(f"Processed and cached theme: {theme_name} ({variant})")
            ...
            if not qss_file.exists():
                self.loading_failed.emit(theme_name, variant, f"Theme file not found: {qss_file}")
                return
            ...
            self.status_update.emit(f"Loaded and cached theme: {theme_name} ({variant})")
            self.theme_loaded.emit(theme_name, variant, stylesheet)
            
        except Exception as e:
            self.loading_failed.emit(theme_name, variant, str(e))
    
    def _convert_resource_paths_to_filesystem(self, stylesheet, theme_name, variant, build_dir):
    ...
    def run(self):
        ...
        for i, theme_name in enumerate(self._themes):
            ...
            for variant in variants:
                ...
                if theme_cache.get(theme_name, variant) or theme_cache.get_raw(theme_name, variant):
                    loaded_count += 1
                    continue
                ...
                try:
                    ...
                    if qss_file.exists():
                        ...
                        processed_stylesheet = loader._convert_resource_paths_to_filesystem(
                            raw_stylesheet, theme_name, variant, build_dir
                        )
